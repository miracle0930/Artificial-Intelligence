...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    for p in self.particles:
      trueDistance = util.manhattanDistance(p, pacmanPosition)
      particles_weight[p] = particles_weight[p] + emissionModel[trueDistance]

    resample = []
    if particles_weight.totalCount() == 0:
      for i in range(self.numParticles):
        resample.append(random.choice(self.legalPositions))
    else:
      for i in range(self.numParticles):
        resample.append(util.sample(particles_weight))
  ...
  def elapseTime(self, gameState):
    ...
    for oldPos in self.particles:
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
      resample.append(util.sample(newPosDist))
      # print(newPosDist)
      # for newPos, prob in newPosDist.items():
      #   allPossible[newPos] += prob * self.beliefs[oldPos]
    self.particles = resample
  ...
  def initializeParticles(self):
    ...
    for i in range(self.numParticles):
      for j in range(self.numGhosts):
          ghost.append(random.choice(self.legalPositions))
          ghostToAdd = tuple(ghost[:])
      self.particles.append(ghostToAdd)
      ghost = []
    ...
    for p in self.particles:
      ...
      for i in range(self.numGhosts):
        if noisyDistances[i] == None:
          ...
          newParticle.append(p[i])
      particles_weight[tuple(newParticle)] += weight

    for pp, weight in particles_weight.items():
      if weight == 0:
        continue
      self.particles = []
      for i in range(self.numParticles):
        self.particles.append(util.sample(particles_weight))
      return
    self.initializeParticles()
