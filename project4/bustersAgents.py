...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    ghost_notcapture = None
    disToGhost = float('inf')
    # calculate the shortest distance to the ghost
    for ghost_living in livingGhostPositionDistributions:
      target_position = ghost_living.argMax()
      dist = self.distancer.getDistance(pacmanPosition, target_position)
      if dist < disToGhost:
        disToGhost = dist
        ghost_notcapture = target_position
    ...
    for action in legal:
      ...
      dist = self.distancer.getDistance(successorPosition, ghost_notcapture)
      if dist < newDist:
        newDist = dist
        nextAction = action
    return nextAction
